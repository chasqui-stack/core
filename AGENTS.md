...
- **Internal conversation read (ADR-011, `app/controllers/conversations.py`):** `GET /conversations/{channel}/{external_id}/messages` — the gateway-facing, `INTERNAL_API_KEY`-protected **read-mirror of `/ingest`**, the one read the core exposes to gateways (everything else is admin/JWT). **Generic** (channel is a path param, no `if channel == …`), read-only (404 on an absent contact — never creates), newest-first, mirroring the admin timeline serialization (`has_media` boolean, **never** embeddings or media payloads). The web channel uses it to rehydrate a visitor's thread on (re)open; any channel can.
- **Tool Registry (§8):** tools are LangChain `@tool`/`StructuredTool` with Pydantic `args_schema`, injected via `ToolNode`/`bind_tools`, accessed through `ToolRuntime[TurnContext]`. Add capabilities as self-contained **modules** under `app/modules/` (a package exposing a module-level `module` attribute — auto-discovered at startup) — never by editing the core. Runtime enable/disable per tool lives in `agent_config.enabled_tools`; tool exceptions become error `ToolMessage`s (`app/services/agent_middleware.py`). Modules can also ship **tables** (`register_models()` — `registry.discover()` runs in `alembic/env.py` + `tests/conftest.py`), **admin routes** (`register_admin_routes()` → JWT-protected `/admin/modules/<name>`), **config knobs** (`config_schema()` ↔ `agent_config.tool_config[config_key]`; keep schemas FLAT — str/int/float/bool only — so the admin auto-renders them as forms) and **system-prompt fragments** (`system_prompt_fragment(context, query)` → one block appended to the prompt each turn, ADR-012 — failure-isolated, `None` = silent; the memory facts block and faq's opt-in question index live here, never hardcoded in the orchestrator). Reference implementation: `app/modules/faq/` (RAG over `faq_entries`).
...
- **The agent turn (`app/services/orchestrator.py`):** LangChain v1 `create_agent` — DB-editable system prompt (`agent_config` singleton) + module prompt fragments (ADR-012 — memory's pgvector facts, faq's opt-in question index) + history + current message (multimodal content blocks gated by `app/core/llm_capabilities.py`). The LLM is swappable via `.env` only (`app/core/llm.py` / `init_chat_model`): google · anthropic · openai · openrouter · ollama.
- **STT fallback (ADR-010, `app/services/transcription.py`):** when the LLM lacks native audio (`caps.audio=False`) and `STT_PROVIDER` is set, a voice note is transcribed to text *before* the turn (`_transcribe_if_needed`, in both `run_turn` and `run_coalesced_turn`) so any model can answer it. OpenAI-compatible client (Groq `whisper-large-v3-turbo` default — native OGG/Opus, no transcoding); **separate `STT_API_KEY`** (LLM provider ≠ audio provider). Best-effort: disabled or any failure → the existing graceful "ask them to type it" fallback, never breaks a turn. Native-audio models (Gemini) skip STT and get the audio block directly.
