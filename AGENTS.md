...
- **Canonical outbound seam (ADR-004, `app/services/channel_send.py`):** operator replies go through each gateway's `POST /send` (mirror of `/ingest`, same `INTERNAL_API_KEY`); the core resolves `CHANNEL_<CHANNEL>_SEND_URL` from `.env` — one var per channel, never a channel SDK in the core. Gateway error `code`s (`WINDOW_EXPIRED`, `NO_WA_ID`, …) pass through to the admin. Handoff notifications (`app/services/notify_service.py`): optional `NOTIFY_WEBHOOK_URL` + optional SMTP (stdlib `smtplib`, any relay — Brevo/Mailgun/SES), both best-effort, fired as background tasks.
